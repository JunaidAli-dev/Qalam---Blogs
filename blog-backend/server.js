...
// FIXED: Add axios import with error handling
let axios;
try {
  axios = require('axios');
...
// FIXED: Image proxy route with proper error handling
  ...
  try {
    ...
    if (!axios) {
      return res.status(500).json({ 
        success: false, 
        error: 'Axios not available. Please check server configuration.' 
      });
    }
...
app.get('/', (req, res) => {
  res.json({ 
    ...
    dependencies: {
      axios: axios ? '✅ Available' : '❌ Missing',
      express: '✅ Available',
      cors: '✅ Available'
    },
    features: {
      imageProxy: axios ? true : false,
      cors: true,
      database: true
    }
  });
...
app.get('/api', (req, res) => {
  res.json({
    ...
    endpoints: {
      ...
      auth: '/api/auth',
      upload: '/api/upload',
      imageProxy: axios ? '/api/proxy-image?url=<image_url>' : 'Not available (axios missing)'
    },
    status: 'active',
    cors: {
      enabled: true,
    ...
    dependencies: {
      axios: axios ? 'Available' : 'Missing - run: npm install axios'
    }
  });
});
...
app.get('/api/status', (req, res) => {
  res.json({
    ...
    services: {
      database: 'connected',
      imageProxy: axios ? 'active' : 'inactive (axios missing)',
      cors: 'enabled'
    },
    dependencies: {
      axios: axios ? 'loaded' : 'missing'
    }
  });
});
...
app.get('/health', (req, res) => {
  res.status(200).json({
    ...
    checks: {
      server: 'healthy',
      database: 'connected',
      imageProxy: axios ? 'operational' : 'disabled',
      axios: axios ? 'available' : 'missing'
    }
  });
});
